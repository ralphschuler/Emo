...
export class ST7789 {
  ...
  constructor(private opts: St7789Options) {
    ...
    this.spi.config({
      mode: opts.spiMode ?? 0,
      bits: 8,
      speedHz: opts.speedHz ?? 31_000_000,
    });
  ...
  private cmd(byte: number) { this.dc.set(0); this.spi.write(Buffer.from([byte & 0xff])); }
  ...
  setAddressWindow(x: number, y: number, w: number, h: number) {
    ...
    this.cmd(CMD.CASET);
    this.data(Buffer.from([(x1>>8)&0xff, x1&0xff, (x2>>8)&0xff, x2&0xff]));
    this.cmd(CMD.RASET);
    this.data(Buffer.from([(y1>>8)&0xff, y1&0xff, (y2>>8)&0xff, y2&0xff]));
    this.cmd(CMD.RAMWR);
  }
  ...
  fillRect(x: number, y: number, w: number, h: number, rgb565: number) {
    ...
    while (sent < total) {
      ...
      this.spi.write(chunk.subarray(0, take * 2));
      sent += take;
    }
  }
